            ...
            <tr class="border-b border-customGray text-center hover:bg-secondary-400 ">
                ...
                <td>
                    <div class="flex items-center justify-end me-5">
                        <a href="{{ route('categories.edit', $category) }}" class="primary-ward-font">
                            <span class="w-duto h-auto text-gray-500 hover:text-gray-700 me-4"><i class="fa-solid fa-pencil h-5 w-5"></i></span>
                        </a>

                        <form onsubmit="return confirm('Do you really want to delete this item?');" method="post" enctype="multipart/form-data" action="{{route('categories.destroy',$category)}}" >
                            @csrf
                            @method('delete')
                            <button>
